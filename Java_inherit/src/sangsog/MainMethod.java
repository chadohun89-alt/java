...
////		Microwave microwave = new Microwave();
		...
		// 메서드를 중점으로 다루는 클래스가 여러개라면 추상 부모 클래스 또는 인터페이스로 묶어준다.
 ...
 * 추상 클래스, 추상 메서드
 *
 * <추상클래스 만드는 이유> 상속 관계에서 공통적인 부분을 부모 클래스에 두는데 자식 클래스들 마다 특징적인 동작을 위해 추상 메소드를
 * 만들어 두어야 한다.
 * 
 * 자식 클래스들에게 규칙을 강제화 하기 위해
 * 
 * 설계도를 만들기 위해
 * 
 * 다형성을 안전하게 쓰기 위해
 * 
 * 
 * <추상 클래스 사용하면 안 되는 경우> - 단순 규칙만 필요할 때 . 공통 코드는 하나도 없고 메서드만 있는 경우 - 여러 클래스에 동시에
 * 적용해야 할 때 . 자바는 상속을 오직 하나만 할 수 있다.
 * 
 * -> 이런 경우는 인터페이스 사용 인터페이스란 추상클래스의 일종이다.
 * 
 */
...
// 상속공부 하기 위해 사용할만한 주제
/*
 * 동물에 대한 상속 직원 급여 계산 시스템 - 직원 (부모) / 정규직, 파트타임 (자식) 결제 시스템 - 결제 (부모) / 현금, 카드,
 * 이체 (자식) 게임 캐릭터 - 캐릭터 (부모) / 직업 (자식) 교통 수단 시스템 - 교통 수단 (부모) / 버스, 자전거, 기차, 차,
 * 오토바이 (자식)
 * 
 * 
 */
