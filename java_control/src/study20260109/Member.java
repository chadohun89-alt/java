...
public class Member {
	...
	/*
	 * 제어자 (뭔가 이름 앞에 붙는 느낌이긴 한데...)
	 * static
	 * 		'공통의'라는 뜻으로 사용되는 제어자이다.
	 * 		static 변수와 static 메서드가 있다.
	 * 		static 메서드에는 static 변수만 사용가능
	 * 				인스턴스 변수는 사용불가
	 * 		1. 모든 객체가 공유해야 하는 데이터가 필요한 경우이거나
	 * 		2. 클래스의 대표 데이터로 사용해야 하는 경우
	 * 		3. 다중 접속에 의해 너무 많은 생성을 방지하기 위해
	 * 		사용하는 경우
	 * 
	 * 
	 * 
	 ...
	 * 		특정 데이터가 실수에 의해 변경 되지 않게 하고자 할 때 사용
	 * 		개발과정에서 다른 팀원이 데이터 변경 시키지 못하게 하기 위한 용도
	 * 
	 * 접근 제어자
	 * 	권한 설정 하는 거, 변수, 메서드, 클래스에 인터페이스 등에 사용된다.
	 * 
	 * 		public
	 *			'공공의'라는 뜻으로 사용되는 제어자
	 *
	 ...
	 * 자바의 변수 종류
	 * 
	 * 인스턴스 변수 : 클래스의 멤버 변수, 객체 생성 시 존재하는 변수
	 * 				객체 소멸 시 같이 소멸 되는 변수
	 * 
